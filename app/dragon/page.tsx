...
const PRODUCTS = {
    tops: [
        { id: 't1', name: 'Dragon Heavy Tee', img: 'https://images.unsplash.com/photo-1583743814966-8936f5b7be1a?auto=format&fit=crop&q=80&w=400' },
        { id: 't2', name: 'Obsidian Tech Hoodie', img: 'https://images.unsplash.com/photo-1556905055-8f358a7a47b2?auto=format&fit=crop&q=80&w=400' },
        { id: 't3', name: 'Cyber Shell Mesh', img: 'https://images.unsplash.com/photo-1618354691373-d851c5c3a990?auto=format&fit=crop&q=80&w=400' },
        { id: 't4', name: 'Imperial Dragon Silk', img: 'https://images.unsplash.com/photo-1596755094514-f87e34085b2c?auto=format&fit=crop&q=80&w=400' },
    ],
    bottoms: [
        { id: 'b1', name: 'Cargo Tech Pants', img: 'https://images.unsplash.com/photo-1624378439575-d8705ad7ae80?auto=format&fit=crop&q=80&w=400' },
        { id: 'b2', name: 'Raw Architecture Denim', img: 'https://images.unsplash.com/photo-1542272454315-4c01d7abdf4a?auto=format&fit=crop&q=80&w=400' },
        { id: 'b3', name: 'Stealth Leather Slacks', img: 'https://images.unsplash.com/photo-1551488852-081bd4c9028c?auto=format&fit=crop&q=80&w=400' },
    ],
    shoes: [
        { id: 's1', name: 'Talon Low Sneakers', img: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&q=80&w=400' },
        { id: 's2', name: 'Scale-Lock Tech Boots', img: 'https://images.unsplash.com/photo-1608231387042-66d1773070a5?auto=format&fit=crop&q=80&w=400' },
        { id: 's3', name: 'Vapor Urban Runners', img: 'https://images.unsplash.com/photo-1606107557195-0e29a4b5b4aa?auto=format&fit=crop&q=80&w=400' },
    ]
};
            ...
            <main className="pt-20 md:pt-28 pb-12 px-4 sm:px-6 max-w-7xl mx-auto w-full flex flex-col lg:flex-row gap-8 lg:gap-12 min-h-screen">
                ...
                <div className="w-full lg:w-5/12 order-2 lg:order-1 flex flex-col gap-6">
                    <div className="bg-slate-900/30 border border-slate-800 rounded-[2.5rem] p-6 md:p-8 backdrop-blur-xl flex-1 flex flex-col shadow-2xl">
                        ...
                        {/* Tabs */}
                        <div className="flex p-1 bg-slate-950/90 rounded-2xl mb-12 border border-white/5 backdrop-blur-xl">
                            {(['tops', 'bottoms', 'shoes'] as const).map((cat) => (
                                <button
                                    key={cat}
                                    onClick={() => setCurrentCategory(cat)}
                                    className={cn(
                                        "flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase transition-all tracking-[0.15em]",
                                        currentCategory === cat
                                            ? "bg-slate-800/60 text-orange-500 border border-white/5"
                        ...
                        {/* Product Grid */}
                        <div className="grid grid-cols-2 gap-5 h-[440px] lg:h-[640px] overflow-y-auto pr-2 custom-scrollbar content-start pb-10">
                            {/* Custom Upload Card */}
                            <div
                                onClick={() => {
                                ...
                                className={cn(
                                    "relative group cursor-pointer rounded-[2.25rem] overflow-hidden border-2 transition-all duration-500 flex flex-col items-center justify-center aspect-[3/4]",
                                    selections[currentCategory]?.id === `custom-${currentCategory}`
                                        ? "card-active z-10 scale-[1.02] ring-4 ring-orange-500/10"
                                        : "border-white/5 bg-slate-900/10 hover:border-orange-500/30 hover:bg-slate-800/20 hover:scale-[1.03]"
                                )}
                            >
                                {customAssets[currentCategory] ? (
                                    <img src={customAssets[currentCategory]!} className="w-full h-full object-cover" />
                                ) : (
                                    <div className="flex flex-col items-center group-hover:text-orange-400 transition-colors p-6 text-center z-10">
                                        <div className="w-14 h-14 bg-slate-800/80 backdrop-blur-xl border border-white/10 rounded-2xl mb-4 flex items-center justify-center group-hover:bg-orange-500/10 group-hover:border-orange-500/30 transition-all duration-500">
                                            <Plus className="h-6 w-6 text-slate-400 group-hover:text-orange-400" />
                                        </div>
                                        <span className="text-[10px] font-black uppercase tracking-[0.25em] leading-relaxed text-slate-500 group-hover:text-orange-400/80 transition-colors">New<br />Visual</span>
                                    </div>
                                )}

                                {/* Glass Overlay for placeholder */}
                                {!customAssets[currentCategory] && (
                                    <div className="absolute inset-0 bg-gradient-to-b from-white/[0.02] to-transparent pointer-events-none"></div>
                                )}

                                {selections[currentCategory]?.id === `custom-${currentCategory}` && (
                                    <div className="absolute top-4 right-4 bg-orange-500 p-2 rounded-full z-20 border-2 border-slate-900">
                            ...
                            {PRODUCTS[currentCategory].map((item) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={cn(
                                            "relative group cursor-pointer rounded-[2.25rem] overflow-hidden border-2 transition-all duration-500 aspect-[3/4] bg-slate-950",
                                            active
                                                ? "card-active z-10 scale-[1.02] ring-4 ring-orange-500/10"
                                                : "border-white/5 hover:border-orange-500/20 hover:scale-[1.03]"
                                        )}
                                    >
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/20 to-transparent p-5 sm:p-6 flex flex-col justify-end">
                                            <div className="translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                                                <p className="text-[10px] font-black text-white uppercase tracking-[0.1em] leading-tight group-hover:text-orange-400 transition-colors">{item.name}</p>
                                                <div className="h-0.5 w-0 group-hover:w-10 bg-orange-500 mt-3 transition-all duration-700 rounded-full"></div>
                                            </div>
                                ...
                                {(['tops', 'bottoms', 'shoes'] as const).map((cat) => {
                                    ...
                                    return (
                                        <div key={cat} className={cn(
                                            "flex-shrink-0 w-14 h-14 rounded-xl border flex items-center justify-center overflow-hidden transition-all",
                                            item ? "border-orange-500 bg-slate-900 shadow-xl" : "border-dashed border-slate-800 bg-slate-950/20"
                                        )}>
                                            {item ? (
                                                <img src={item.img} className="w-full h-full object-cover" />
                                            ) : (
                                                <span className="text-[8px] text-slate-700 font-black uppercase">{cat[0]}</span>
                                            )}
                                        </div>
                                    );
                                })}
                    ...
                    <button
                        ...
                        className="relative w-full py-6 rounded-[2rem] font-black text-lg uppercase tracking-[0.3em] transition-all duration-500 overflow-hidden group btn-gradient shadow-2xl active:scale-95 disabled:opacity-50"
                    >
